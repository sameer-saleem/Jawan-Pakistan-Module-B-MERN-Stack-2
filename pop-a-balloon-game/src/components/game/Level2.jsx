// src/components/game/Level2.jsx
...
export default function Level2({ onLevelComplete }) {
  ...
  // Spawn balloons with movement patterns
  useEffect(() => {
    ...
    spawn();
    const interval = setInterval(spawn, 1200); // Faster spawn than level 1
  ...
  const handleHover = (id, reactionTime, colorObj) => {
    ...
    if (isCorrect) {
      ...
      if (scoreRef.current >= 12) { // Higher win condition for level 2
        endGame(true);
      }
    } else {
      setLives(l => {
  ...
  return (
    <Box sx={{ position: "relative", height: "80vh", overflow: "hidden", background: "#87CEEB" }}>
      ...
      <Alert severity="info" sx={{ position: "absolute", top: 10, left: "50%", transform: "translateX(-50%)", zIndex: 100, fontSize: 18, fontWeight: "bold" }}>
        LEVEL 2: WATCH THE MOVEMENT PATTERNS!
      </Alert>
      ...
      <Box sx={{ position: "absolute", top: 150, left: 20, background: "rgba(255,255,255,0.95)", p: 3, borderRadius: 3, zIndex: 100 }}>
        <Typography fontSize={24} fontWeight="bold">Level 2</Typography>
        <Typography fontSize={24} fontWeight="bold">Score: {score}/12</Typography>
      ...
      {gameOver && (
        <Box sx={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <Typography variant="h2" color="white" fontWeight="bold">
            {lives > 0 ? "LEVEL 2 COMPLETE!" : "GAME OVER"}
          </Typography>
        </Box>
      )}
    </Box>
